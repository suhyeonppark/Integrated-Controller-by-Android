...
/// One controllable device, built by pairing the user's ON/OFF buttons that
/// target the same relay (CE-REL8) or IR port (CE-IRS4). Drives the card grid.
class DeviceGroup {
  const DeviceGroup({
    required this.name,
    required this.icon,
...
/// Pairs a flat button list into devices, keyed by relay number / IR port,
...
List<DeviceGroup> groupDevices(List<ButtonConfig> buttons) {
  ...
  for (final b in buttons) {
    final key = b.type == ButtonType.relay ? 'r${b.relay}' : 'p${b.irPort}';
...
DeviceGroup _buildDevice(List<ButtonConfig> members) {
  final isRelay = members.first.type == ButtonType.relay;
  ButtonConfig? on;
  ButtonConfig? off;
  if (isRelay) {
    for (final b in members) {
      if (b.relayMode == RelayMode.latchClose) {
        on ??= b;
      } else if (b.relayMode == RelayMode.latchOpen) {
        off ??= b;
      }
    }
  }
  return DeviceGroup(
    ...
    icon: _deviceIcon(_deviceName(members), isRelay),
    members: members,
    onButton: on,
    offButton: off,
...
IconData _deviceIcon(String name, bool isRelay) {
  ...
  return isRelay ? Icons.electrical_services_rounded : Icons.settings_remote_rounded;
}
...
class DeviceCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bool? isOn = device.relay != null
        ? state.relayIsOn(device.relay!)
        : null;
